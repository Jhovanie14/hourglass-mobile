...
export function useCallRecords() {
  ...
  const insertInbound = useCallback(
    async (
      callerNumber: string,
  ...
  const insertOutbound = useCallback(
    async (phoneNumberId: string, to: string): Promise<string | null> => {
      const { data: call, error } = await supabase
        .from("calls")
        .insert({
  ...
  const markAnswered = useCallback(
    async (callId: string, startedAt: string): Promise<void> => {
